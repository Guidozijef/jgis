...
export class Roaming {
  ...
  _ComputeRoamingLineProperty(Lines, time) {
    ...
    for (let i = 0; i < lineLength; i++) {
      let time = Cesium.JulianDate.addSeconds(start, (i * tempTime) / lineLength, new Cesium.JulianDate())
      if (i == lineLength - 1) {
        time = stop
      }
        ...
        if (_this.viewer.clock.shouldAnimate) {
          ...
          if (_this.ifAffixedTo) {
            ...
            Cesium.sampleTerrainMostDetailed(terrainProvider, [
              Cesium.Cartographic.fromDegrees((geoPt1.longitude / Math.PI) * 180, (geoPt1.latitude / Math.PI) * 180)
            ]).then(function (updatedPositions) {
              _this.entity.position.addSample(
                _this.viewer.clock.currentTime,
                Cesium.Cartesian3.fromRadians(updatedPositions[0].longitude, updatedPositions[0].latitude, updatedPositions[0].height)
              )
            })
          }
          if (_this.ifTileset) {
  ...
  changingView(value, options) {
    ...
    if (value == 1) {
      ...
      this.viewer.zoomTo(this.viewer.entities, new Cesium.HeadingPitchRange(Cesium.Math.toRadians(-90), Cesium.Math.toRadians(-15), 8000))
    } else if (value == 4) {
      this.viewer.zoomTo(
